...
main(int argc, char* argv[])
{
	...
	if (me == BOSS)
	{
		for (int dst = 0; dst < NumCpus; dst++)
		{
			...
			MPI_Send(&BigSignal[dst * PPSize], PPSize + MAXSHIFTS, MPI_FLOAT , dst , 0, MPI_COMM_WORLD ); //change 1
		}
	}
	else
	{
		MPI_Recv(PPSignal, PPSize + MAXSHIFTS, MPI_FLOAT, BOSS, 0, MPI_COMM_WORLD, &status); // change 2
	}
	...
	{
		MPI_Send(PPSums, MAXSHIFTS, MPI_FLOAT, BOSS, 0, MPI_COMM_WORLD); // change 3
	}
	...
	if (me == BOSS)
	{
		...
		for (int src = 0; src < NumCpus; src++)
		{
			...
			MPI_Recv(tmpSums, MAXSHIFTS, MPI_FLOAT, src, 0, MPI_COMM_WORLD, &status); // change 4
			for (int s = 0; s < MAXSHIFTS; s++)
				BigSums[s] += tmpSums[s];
		}
	}
